...
大家还可以使用LTspice中提供的BSIM模型，例如BSIM3v3（level=8）以及BSIM4.6.1（level=14），这两个模型的使用非常简单，直接将上述txt文件中的代码替换为以下代码即可（这里我是用LEVEL=14作为例子）：
...
+LEVEL=14
```
选择该模型的原因有2点，其一，BSIM4模型很多参数并没有省略为0（我们在后续的动态电路课程会使用），相对比较准确，其二，根据BSIM小组官网（https://www.bsim.berkeley.edu/）介绍，BSIM4模型可以支持0.13 um, 90 nm, 65 nm, 45/40 nm, 23/28 nm, 甚至是 22/20nm这样先进的工艺！
**注意：我仅测试过实验7实验内容没有问题，但是并不确定其他实验是否依然正确**
### 你知道吗？
大家可以去搜索BSIM3v3的手册，在手册作者的最后一位，是一位华人科学家，他就是胡正明，大家可以搜一下胡正明教授的故事，可以说，胡教授让摩尔定律得以延续下去。在接下来的旅程中，我们还会再次遇到胡教授。
